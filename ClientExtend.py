...
class ClientExtend:
    ...
    def createWidgets(self):
        ...
        self.totaltimeBox = Label(self.master, width=12, text="00:00", bg= "#A5D2EB")
        self.totaltimeBox.grid(row=1, column=3, columnspan=1, padx=5, pady=5)

        # Create a label to display remaining time of the movie

        self.remainTimeBox = Label(self.master, width=12, text="00:00", bg="#A5D2EB")
        ...
        self.forward.grid(row=3, column=2, padx=2, sticky= E + W, pady=2)
        ...
        self.backward.grid(row=3, column=1, sticky = E + W, padx=2, pady=2)


        self.scrollbar = Scale(self.master, from_=0, to=100,
                                       length=300, orient=HORIZONTAL,
                                       showvalue=0, sliderlength=15,
                                       activebackground="yellow", bg="gray", troughcolor="black")
        self.scrollbar.grid(row=1, column=0, columnspan=5, padx=2, pady=2)
        # self.scrollbar['command'] = self.scrollMovie


    def scrollMovie(self):
        self.sendRtspRequest(self.SCROLLING)
            ...
            try:
                ...
                if data:
                    ...
                    try:
                        ...
                        self.totaltimeBox.configure(text="%02d:%02d" % (self.totalTime // 60, self.totalTime % 60))
                        self.remainTimeBox.configure(text="%02d:%02d" % ((self.totalTime - self.currentTime)// 60, (self.totalTime - self.currentTime) % 60))
                        self.scrollbar.set(int(self.currentTime))
            ...
            if self.sessionId == session:
                if int(lines[0].split(' ')[1]) == 200:
                    if self.requestSent == self.SETUP:
                        ...
                        # self.state = ...
                        self.scrollbar['to'] = int(self.totalTime)
